...
export class HUD {
  constructor(scene) {
    ...
    // Coins text
    this.coinsText = scene.add.text(46, 12, '0', {
      fontFamily: 'Arial',
      fontSize: '20px',
      fontStyle: 'bold',
      color: '#ffd23c',
    ...
    // Population
    this.popText = scene.add.text(220, 12, 'Pop: 0', {
      fontFamily: 'Arial',
      fontSize: '16px',
      color: '#ddeeff',
    }).setScrollFactor(0).setDepth(901);
    ...
    // Bonus
    this.bonusText = scene.add.text(220, 30, 'Bonus: x1.0', {
      fontFamily: 'Arial',
      fontSize: '13px',
      color: '#aaccee',
    }).setScrollFactor(0).setDepth(901);
    ...
    // Income
    this.incomeText = scene.add.text(380, 12, 'Income: +0', {
      fontFamily: 'Arial',
      fontSize: '16px',
      color: '#88ddaa',
    }).setScrollFactor(0).setDepth(901);
    ...
    // Rebirths
    this.rebirthText = scene.add.text(380, 30, '', {
      fontFamily: 'Arial',
      fontSize: '13px',
      color: '#ddaaff',
    }).setScrollFactor(0).setDepth(901);
    ...
    // Shop button
    this.shopBtn = scene.add.graphics();
    this.shopBtn.setScrollFactor(0);
    this.shopBtn.setDepth(901);
    this.drawShopButton(false);

    this.shopBtnZone = scene.add.zone(WIDTH - 70, 22, 100, 34)
      .setScrollFactor(0)
      .setDepth(902)
    ...
    this.rebirthBtnZone = scene.add.zone(WIDTH - 155, 22, 65, 28)
      .setScrollFactor(0)
      .setDepth(902)
      .setInteractive({ useHandCursor: true })
      .setVisible(false);
  ...
  drawShopButton(hover) {
    ...
    // "Shop" text (using graphics since text would be separate)
    if (!this.shopLabel) {
      this.shopLabel = this.scene.add.text(WIDTH - 70, 15, 'Shop', {
        fontFamily: 'Arial',
        fontSize: '16px',
        fontStyle: 'bold',
        color: '#ffffff',
